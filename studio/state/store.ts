...
export function createStore(deps: StoreDeps): StudioStore {
   ...
   *  Offline-safe: reads refs on disk, never fetches, so a deleted-upstream ref can still read as
   *  present until the next fetch. */
  async function refExists(ref: string): Promise<boolean> {
    return (await git.git(["rev-parse", "--verify", "--quiet", ref], { cwd: repoRoot })).code === 0;
  }

  /** Whether `origin/<branch>` exists as a remote-tracking ref. */
  async function hasOriginRef(branch: string): Promise<boolean> {
    return refExists(`refs/remotes/origin/${branch}`);
  }
